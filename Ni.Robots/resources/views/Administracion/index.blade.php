    ...
    <div class="w-full space-y-6">
        ...
        @if (!empty($alerts))
            <div class="flex flex-col md:flex-row gap-2 w-full">
                @foreach ($alerts as $alert)
                    <div
                        class="flex-1 p-4 rounded-lg border-l-4 
            @if ($alert['type'] === 'warning') bg-yellow-50 border-yellow-400 text-yellow-700 dark:bg-yellow-900 dark:bg-opacity-20 dark:text-yellow-300 
            @elseif($alert['type'] === 'info') 
                bg-blue-50 border-blue-400 text-blue-700 dark:bg-blue-900 dark:bg-opacity-20 dark:text-blue-300 
            @elseif($alert['type'] === 'success') 
                bg-green-50 border-green-400 text-green-700 dark:bg-green-900 dark:bg-opacity-20 dark:text-green-300 
            @else 
                bg-gray-50 border-gray-400 text-gray-700 dark:bg-gray-900 dark:bg-opacity-20 dark:text-gray-300 @endif
            ">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 dark:bg-gray-800">
                    ...
                    <div class="p-3 bg-white bg-opacity-20 rounded-full">
                        <svg class="w-8 h-8 text-gray-800 dark:text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="mt-5 flex items-end justify-between">
                    <div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">Ventas Totales</span>
                    ...
                    <span
                        ...
                        <svg class="fill-current dark:text-white " width="12" height="12" viewBox="0 0 12 12"
                        ...
                        <span class="text-xs">+${{ number_format($salesThisMonth ?? 0, 2) }} este mes</span>
                    </span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                ...
                <div class="mt-5 flex items-end justify-between">
                    ...
                    <span
                        ...
                        <span class="text-xs">{{ $activeUsers ?? 0 }} activos</span>
                    </span>
                </div>
            </div>
            {{-- Productos --}}
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                ...
                <div class="mt-5 flex items-end justify-between">
                    <div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">Productos Totales</span>
                        <h4 class="mt-2 text-title-sm font-bold text-gray-800 dark:text-white/90">
                            {{ $totalProducts ?? 0 }}
                        </h4>
                    </div>
                    <span
                        ...
                        <span class="text-xs">{{ $lowStock ?? 0 }} con bajo stock</span>
                    </span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                ...
                <div class="mt-5 flex items-end justify-between">
                    ...
                    <span
                        ...
                        <span class="text-xs">{{ $completedAppointments ?? 0 }} completadas</span>
                    </span>
                </div>
            </div>
        </div>

        {{-- 🔹 PRIMERA FILA DE GRÁFICOS - Ocupa todo el ancho --}}
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2 sm:mb-0">Tendencia de Ventas - Últimos
                        7 Días</h3>
                    <div class="flex items-center space-x-2">
                        <span class="text-sm text-gray-500 dark:text-gray-400">Ventas totales:
                            ${{ number_format($totalSales ?? 0, 2) }}</span>
                    </div>
                </div>
                <div class="h-80">
        ...
        <h2 class="text-2xl text-gray-800 dark:text-white font-bold text-start">Ciudades del pais con mas Usuarios</h2>
        <div class=" relative flex flex-col mt-4 md:flex-row justify-evenly items-center gap-x-1">

            <div id="map-legend" class="mt-6     bg-gray-50 dark:bg-gray-800 rounded-lg p-4  ">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-3">
                    Leyenda de Usuarios por Ciudad
                </h3>
                <ul id="legend-list" class="space-y-2"></ul>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', async () => {
                try {
                    ...
                    // Calcular máximo para escalar colores
                    const max = Math.max(...Object.values(data));

                    Object.entries(data).forEach(([ciudad, cantidad]) => {
                        const id = mapa[ciudad];
                        const path = document.getElementById(id);
                        if (!path) return;

                        // Escala de color tipo "verde → rojo"
                        const porcentaje = cantidad / max;
                        const r = Math.floor(255 * porcentaje);
                        const g = Math.floor(180 * (1 - porcentaje));
                        const b = 80;
                        ...
                        item.className =
                            "flex items-center justify-between bg-white dark:bg-gray-700 px-3 py-2 rounded-lg shadow-sm";
                        item.innerHTML = `
                <div class="flex items-center gap-3">
                    <div class="w-6 h-6 rounded" style="background:${color}"></div>
                    <span class="text-gray-800 dark:text-gray-100 font-medium">${ciudad}</span>
                </div>
                <span class="text-gray-700 dark:text-gray-300 font-semibold">${cantidad} usuarios</span>
            `;
        ...
        {{-- 🔹 SEGUNDA FILA DE GRÁFICOS - 2 columnas grandes --}}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {{-- Métricas de Rendimiento --}}
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">Métricas de Rendimiento</h3>
                <div class="h-80">
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                <div class="space-y-4">
                    <div
                        class="flex items-center justify-between p-4 bg-green-50 rounded-xl dark:bg-green-900 dark:bg-opacity-20">
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 flex items-center justify-center bg-green-100 text-green-600 rounded-lg mr-4 dark:bg-green-800 dark:text-green-200">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white">Nueva Venta Realizada</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">$1,250.00 - Cliente #1234</p>
                            </div>
                        </div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">Hace 2h</span>
                    </div>

                    <div
                        class="flex items-center justify-between p-4 bg-blue-50 rounded-xl dark:bg-blue-900 dark:bg-opacity-20">
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 flex items-center justify-center bg-blue-100 text-blue-600 rounded-lg mr-4 dark:bg-blue-800 dark:text-blue-200">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"
                                        clip-rule="evenodd" />
                                </svg>
                            ...
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white">Nuevo Usuario Registrado</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Usuario: juan.perez</p>
                            </div>
                        </div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">Hace 4h</span>
                    </div>

                    <div
                        class="flex items-center justify-between p-4 bg-purple-50 rounded-xl dark:bg-purple-900 dark:bg-opacity-20">
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 flex items-center justify-center bg-purple-100 text-purple-600 rounded-lg mr-4 dark:bg-purple-800 dark:text-purple-200">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                                        d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"
                                        clip-rule="evenodd" />
                            ...
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white">Cita Médica Programada</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Dr. García - 15:00 hrs</p>
                            </div>
                        </div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">Hace 6h</span>
                    </div>
                </div>
            </div>
        ...
        {{-- 🔹 TERCERA FILA DE GRÁFICOS - 2 columnas --}}
        <div class="grid grid-cols-1 gap-4">
            {{-- Doctores por Especialidad --}}
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">Doctores por Especialidad</h3>
                <div class="h-80">
        ...
        {{-- 🔹 CUARTA FILA - Contenido adicional --}}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {{-- TOP PRODUCTOS --}}
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">Top 5 Productos en Stock</h3>
                    <span class="text-sm text-gray-500 dark:text-gray-400">Más existencias</span>
                </div>
                <div class="space-y-4">
                    @forelse($topProducts ?? [] as $index => $product)
                        <div
                            class="flex items-center justify-between p-4 bg-gray-50 rounded-xl dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200">
                            <div class="flex items-center">
                                <div
                                    class="w-10 h-10 flex items-center justify-center bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-lg font-bold mr-4">
                                    {{ $index + 1 }}
                                </div>
                                <div>
                                    <p class="font-semibold text-gray-900 dark:text-white">{{ $product->nombre_prod }}</p>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">SKU: {{ $product->id ?? 'N/A' }}
                                    </p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $product->existencias }}</p>
                    ...
                    @empty
                        <div class="text-center py-8 text-gray-500 dark:text-gray-400">
                            ...
                            <p class="text-lg">No hay productos para mostrar</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">Distribución de Ventas</h3>
                <div class="h-80">
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">Ventas por Fabricante</h3>
                <div class="h-80">
                    <canvas id="salesByManufacturerChart"></canvas>
                </div>
            </div>
        </div>
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            console.log('Dashboard cargado - Inicializando gráficos Chart.js...');
            ...
            const salesByManufacturerData = @json($salesByManufacturer ?: []);
            const doctorsBySpecialtyData = @json($doctorsBySpecialty ?: []);
            ...
            function createSampleData() {
                ...
                const sampleSalesTrend = {
                    labels: days,
                    data: [3200, 4100, 3500, 5200, 4800, 6100, 5500]
                };

                const sampleSalesByManufacturer = {
                    labels: ['MediCorp', 'HealthTech', 'BioSolutions', 'PharmaPlus'],
                    data: [12500, 8900, 6700, 4300]
                };

                const sampleDoctorsBySpecialty = {
                    labels: ['Cardiología', 'Pediatría', 'Dermatología', 'Ortopedia', 'Neurología'],
                    data: [8, 6, 4, 5, 3]
                };

                return {
                    salesTrend: sampleSalesTrend,
                    salesByManufacturer: sampleSalesByManufacturer,
                    doctorsBySpecialty: sampleDoctorsBySpecialty
                };
            }

            // Determinar si usar datos reales o de ejemplo
            let chartData;
            if (salesTrendData && Object.keys(salesTrendData).length > 0) {
                const trendLabels = Object.keys(salesTrendData).map(date => {
                ...
                chartData = {
                    salesTrend: {
                        labels: trendLabels,
                        data: Object.values(salesTrendData).slice(-7)
                    },
                    salesByManufacturer: {
                        labels: Object.keys(salesByManufacturerData),
                        data: Object.values(salesByManufacturerData)
                    },
                    doctorsBySpecialty: {
                        labels: doctorsBySpecialtyData.map(item => item.especialidad),
                        data: doctorsBySpecialtyData.map(item => item.total)
                    }
                };
            } else {
            ...
            // 1. GRÁFICO DE TENDENCIA DE VENTAS (Línea)
            try {
            ...
            // 2. MÉTRICAS DE RENDIMIENTO (Doughnut)
            try {
                const performanceCtx = document.getElementById('performanceMetricsChart').getContext('2d');
                new Chart(performanceCtx, {
                    type: 'doughnut',
                    data: {
                        labels: ['Ventas Mensuales', 'Usuarios Activos', 'Stock Saludable',
                            'Citas Completadas'
                        ],
                        datasets: [{
                            data: [85, 72, 90, 78],
                ...
                console.error('Error en gráfico de métricas:', error);
            }

            // 3. VENTAS POR FABRICANTE (Bar)
            try {
                const manufacturerCtx = document.getElementById('salesByManufacturerChart').getContext('2d');
                new Chart(manufacturerCtx, {
                    type: 'bar',
                    data: {
                        labels: chartData.salesByManufacturer.labels,
                        datasets: [{
                            label: 'Ventas Totales',
                            data: chartData.salesByManufacturer.data,
                            backgroundColor: [
                                colors.blue,
                                colors.green,
                                colors.orange,
                ...
                console.error('Error en gráfico de fabricantes:', error);
            }

            // 4. DOCTORES POR ESPECIALIDAD (Bar Horizontal)
            try {
                const doctorsCtx = document.getElementById('doctorsBySpecialtyChart').getContext('2d');
                new Chart(doctorsCtx, {
                    type: 'bar',
                    data: {
                        labels: chartData.doctorsBySpecialty.labels,
                        datasets: [{
                            label: 'Cantidad de Doctores',
                            data: chartData.doctorsBySpecialty.data,
                ...
                console.error('Error en gráfico de doctores:', error);
            }

            // 5. DISTRIBUCIÓN DE VENTAS (Pie)
            try {
                const distributionCtx = document.getElementById('salesDistributionChart').getContext('2d');
                new Chart(distributionCtx, {
                    type: 'pie',
                    data: {
                        labels: ['Ventas Este Mes', 'Ventas Meses Anteriores'],
                        datasets: [{
                            data: [{{ $salesThisMonth ?? 15000 }},
                                {{ max(0, ($totalSales ?? 35000) - ($salesThisMonth ?? 15000)) }}
                            ],
                ...
                console.error('Error en gráfico de distribución:', error);
            }

            console.log('🎉 Todos los gráficos Chart.js inicializados correctamente');
        });
    </script>
@endpush
