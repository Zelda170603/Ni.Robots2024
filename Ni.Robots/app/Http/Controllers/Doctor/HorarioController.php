...
{   
    ...
    public function edit() {
        ...
        $horarios = Horarios::where('doctor_id', auth()->id())->get();
        ...
        return view('Administracion.horario', compact('days','horarios'));
    ...
    public function store(Request $request){
        ...
        for($i=0; $i<7; ++$i){
            ...
            if($afternoon_start[$i] > $afternoon_end[$i]){
                $errors[]="Inconsistencia en el intervalo de las horas del turno de la de la tarde del dia ".$this->days[$i].".";
            }
            Horarios::updateOrCreate(
                [
                    'day' => $i,
                    'doctor_id' => Auth::user()->role->roleable_id
                ],
                [
        ...
        $notification= 'El información del horario se ha actualizado exitosamente.';
        //return redirect('/horario')->with(compact('notification'));
        return back()->with(compact('notification'));
    }

    public function update(Request $request, string $days){
